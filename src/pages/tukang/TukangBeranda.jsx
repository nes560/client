...
const TukangBeranda = () => {
  ...
  // 1. URL API (Pastikan sama dengan backend)
  ...
  const [stats, setStats] = useState({
      income: 0,
      totalOrders: 0,
      rating: 4.9 // Hardcoded sementara (bisa diupdate jika ada fitur rating)
  });
    ...
    const fetchData = async () => {
        try {
            ...
            if (result.success) {
                ...
                // Cari pesanan yang statusnya 'Diproses' (Ambil yang paling baru/id terbesar)
                ...
                // Hitung Pendapatan (Default 50rb jika harga null)
    ...
    <div className="pb-24 md:pb-0">
      ...
      <div className="p-6 md:p-0 space-y-6 md:grid md:grid-cols-2 md:gap-6 md:space-y-0">
        ...
        <div>
            ...
            {loading ? (
                <div className="text-center py-10 text-slate-400 text-sm bg-white rounded-3xl border border-slate-100">
                    Memuat data...
                </div>
            ) : activeJob ? (
                // JIKA ADA JOB AKTIF (Status: Diproses)
                <div className="bg-white p-5 rounded-3xl border border-slate-100 shadow-sm relative hover:shadow-md transition">
                    <span className="absolute top-5 right-5 px-3 py-1 bg-green-100 text-green-700 text-[10px] font-bold rounded-full uppercase">
                        Sedang Jalan
                    </span>
                ...
                <div className="flex flex-col items-center justify-center text-center py-8 bg-white border border-dashed border-slate-200 rounded-3xl">
                    ...
                    <p className="text-xs text-slate-400 mt-1">Cek tab Orderan Baru untuk mengambil job</p>
